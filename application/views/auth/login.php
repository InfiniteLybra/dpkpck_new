...
<body>
					...
					<div class="col-lg-6 mb-5 mb-lg-0 position-relative hahaha">
						...
						<div class="card bg-glass rounded-4">
							<div class="card-body px-4 py-5 px-md-5">
								...
								<form id="loginForm" action="<?= base_url('auth/proses_log') ?>" method="post">
									<!-- Email input -->
									...
									<div style="margin: 0 auto; display: flex; flex-direction: column; align-items: center; justify-content: center;">
										...
										<p class="form-label mt-2" id="recaptchaError" style="color: red;"></p>
										<button type="submit" class="btn btn-block mt-3 rounded btn-success" onclick="validateForm()">Login</button>
									</div>
	...
	<script>
		// Validasi saat formulir dikirim
		document.getElementById("loginForm").addEventListener("submit", function(event) {
			var response = grecaptcha.getResponse();
			var errorElement = document.getElementById("recaptchaError");

			if (response.length === 0) {
				// Tampilkan pesan kesalahan di bawah elemen reCAPTCHA
				errorElement.textContent = "Silakan Lengkapi CAPCTHA!";
				event.preventDefault(); // Mencegah formulir dikirim jika reCAPTCHA tidak diisi
			} else {
				// Bersihkan pesan kesalahan jika reCAPTCHA diisi
